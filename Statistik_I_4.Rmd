...
* Verteilungen einer Variable visualisieren 
  ...
  * Histogramm (numerische Variable)
  * Balkendiagramm (kategoriale Variable)

**Multivariat:**

* Ausprägung einer Variablen innerhalb Kategorien einer anderen Variable (Gruppenvergleich)

  * Balkendiagramm (Höhe des Balkens z.B. Mittelwert)
  * Boxplot (Median und IQR)
...
set.seed(123)
N = 12
df = data.frame(ID = c(paste0(rep(1:N)), "13"),
...
boxplot(Tremor~Entzug,data = df2)
```
]
]
...
2 numerische Variablen Nachtschlaf (in Stunden) und Leistung in einem Konzentrationstest (0-150 Punkte)
...
Erstellen Sie ein Streudiagramm für die beobachteten numerischen Lerndauer (in Stunden) und Prüfungsleistung (1-60 Punkte) 
...
2 Kategoriale Variablen: (1) Entzugsstatus heroinabhängiger Patienten (vor dem Entzug = A, während des Entzugs = B) und (2) Tremor (ja/nein)
