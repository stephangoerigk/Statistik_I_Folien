...
### Einheit 6: Stichprobe, Grundgesamtheit und Stichprobenfehler
##### 23.11.2022 | Dr. phil. Stephan Goerigk

---
class: top, left
### Stichprobe, Grundgesamtheit und Stichprobenfehler
...
Wir setzen voraus, dass jede Person i in der Population die gleiche Wahrscheinlichkeit hat, gezogen zu werden. Das heißt, alle Elementarereignisse haben die gleiche Wahrscheinlichkeit:
...
* Sei nun X eine Zufallsvariable, die den Wert 1 annimmt, falls die zufällig gezogene Person depressiv ist, und 0, falls sie es nicht ist.
...
* $X$ eine Zufallsvariable ist, die den Wert 1 annimmt, falls die gezogene Person depressiv ist, und 0, falls nicht,

folgt X einer Bernoulli-Verteilung und der Wert des Parameters $𝝅$ dieser Bernoulli-Verteilung ist identisch mit dem Wert der relativen Häufigkeit $h_{A}$ der Depression in der Population.

* Wenn wir herausfinden wollen, wie hoch die relative Häufigkeit der Depression in der Population ist, müssen wir lediglich herausfinden, welchen Wert der Parameter $𝜋$ hat.
...
* Man kann dann beweisen, dass diese Zufallsvariable $X$ einer Normalverteilung folgt und
  * der Parameter $𝜇$ dieser Normalverteilung dem Mittelwert des IQs in der Population entspricht:
...
* Beispiel: Wir gehen auf Basis unserer Stichprobe davon aus, dass der Parameter $𝜋$
gleich 0.45 ist.
...
* Wir wollen auf Basis der Realisationen der ZVs für jeden Parameter jeweils einen Schätzwert bestimmen, d.h. einen Wert, von dem wir auf der Basis unserer Stichprobe davon ausgehen, dass er „möglichst nahe“ an dem wahren Parameterwert bzw. dem wahren Wert der Maßzahl in der Population liegt.
...
* Angenommen, wir würden uns für h! in dieser Population interessieren, könnten die Population aber nicht direkt untersuchen, sondern nur einfache Zufallsstichproben der Größe n = 10 ziehen.
...
Inferenzstatistik verwendet Stichprobenkennwerte (Stichprobenmittelwert, Stichprobenvarianz oder auch den Quotient zweier Stichprobenvarianzen) als Schätzfunktionen für die entsprechenden Populationsparameter.

* Zieht man immer wieder voneinander unabhängige Zufallsstichproben und berechnet die Kennwerte, werden die einzelnen Kennwerte aufgrund der zufälligen Zusammensetzung der Stichproben nicht ident sein sondern streuen zufällig um den Erwartungswert. Die Kennwerte einzelner Zufallsstichproben sind Realisierungen von Zufallsvariablen (z.B. $X^2$).
...
m[1:20,] %>%
  kbl() %>%
  kable_classic(full_width = T, position = "left", font_size = 12)
```
...
* gegeben sei eine normalverteilte Variable X mit Mittelwert von 11 und Standardabweichung von 5.53
...
$$z=\frac{14.5-11}{5.53}=1.49$$
* In der z-Tabelle schlagen wir nach, wie wahrscheinlich ein z-Wert von mindestens 1.49 ist
...
* Wie hoch ist die Wahrscheinlichkeit das X mehr als 14.5 Punkte aufweist?
...
1.  Wir ziehen unabhängige Zufallsstichproben vom Umfang $n$ ziehen und berechnen. in jeder Teilstichprobe den Mittelwert
...
* Der Wert für $1-\frac{α}{2} = 1-\frac{.05}{2}=1.96$ muss in der z-Tabelle nachgesehen werden. $α$ wird halbiert, da sich die 5% Fehler auf die untere und obere Grenze des KIs aufteilen.
...
$$\hat{s}^2=\frac{\sum\limits _{i=1}^{n}(x_{i}-\bar{x})^2}{n-1}$$
...
* mit deren Hilfe wir Parameter (gesuchter Parameter einer Verteilung entspricht interessierte Größe in der Population) aus Stichprobendaten schätzen
]
]
