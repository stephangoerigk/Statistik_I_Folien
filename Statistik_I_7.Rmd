...
* Was für "Aussagen", die wir über die Population sind gemeint?
...
* Statistisch zu prüfende Aussagen: Hypothesen (kennen wir bereits)
...
* Trifft eine Hypothese zu sprich man oft vom Vorliegen eines **Effekts**
...
* Um alle Wahrscheinlichkeiten für einen Ausgang des Hypothesentests abzudecken formuliert man ein Hypothesenpaar
  * $H_{0}$: Der hypothetisierte Effekt liegt nicht vor
  * $H_{1}$: Der hypothetisierte Effekt liegt vor
...
* Wahrscheinlichkeitsverteilung zur Wahrscheinlichkeit von Wahrscheinlichkeiten: Binomialverteilung (BV)
...
* Merke: 
  * Bei rerichteten Formulierungen wie "mindestens" oder "höchstens" nutzen wir die **kumulierte Binomialverteilungstabelle**
  * Bei unrerichteten Formulierungen wie "genau" nutzen wir die **einfache Binomialverteilungstabelle**
...
Kritischer Wert kann in der kumulierte Binomialverteilungstabelle nachgesehen werden ([**Link zur Tabelle**](https://www.hs-merseburg.de/fileadmin/Hochschule_Merseburg/Extra/Hagenloch/Lehre/Tabelle_Binomialverteilung.PDF))
...
* Kritischer Wert kann in der kumulierte Binomialverteilungstabelle nachgesehen werden ([**Link zur Tabelle, S.6)**](https://www.hs-merseburg.de/fileadmin/Hochschule_Merseburg/Extra/Hagenloch/Lehre/Tabelle_Binomialverteilung.PDF))
...
#### Beispiel 2: Binomialtest (einseitiges Testen) mit NV-Approximation

* Binomialverteilung approximiert bei großem N die Normalverteilung

* Wenn Voraussetzungen für NV-Approximation der Binomialverteilung gegeben $μ=np$, $σ^2 =np(1−p)$

* Wir können also Wahrscheinlichkeiten in z-Wert umrechenen und diese in der Standardnormalverteilungstabelle ablesen

Standardnormalverteilte Teststatistik:

$$z=\frac{K-μ}{σ}=\frac{K-np_{0}}{\sqrt{np_{0}(1-np_{0})}}$$
...
* Eine Psychologin vermutet, dass weniger als 50% der Frauen, die eine Diät machen übergewichtig sind
...
In einer von ihr erhobenen Stichprobe von $N=200$ Frauen, die derzeit Diät halten, sind 87 übergewichtig.
...
Berechnung Teststatistik z:

* $μ=np=100$, $k= 87$

$$z=\frac{K-np_{0}}{\sqrt{np_{0}(1-np_{0})}}=\frac{87-100}{\sqrt{200 \cdot 0.5 \cdot 0.5}}=-1.77$$

Nachschlagen in der Standardnormalverteilungstabelle:

$$P(Z \leq -1.77) = 0.0384$$
$$0.0384< \alpha= 0.05$$
Verwerfung der $H_{0}$ zugunsten der $H_{1}$ mit einem Risiko für einen Fehler erster Art von 0.0384
...
ggplot(data.frame(x = c(-4, 4)), aes(x)) +
  ...
  labs(x = "", y = "") +
  annotate(geom = "text", x = -2.5, y = 0.05, label = "5%", size = 6) +
  scale_x_continuous(breaks = c(-4,-1.77,0,2,4)) +
  geom_vline(xintercept = -1.77, linetype = "dotted", colour = "red") +
  ggtitle("Linksseitiger Test") +
   theme_classic() +
  theme(text = element_text(size = 25), axis.text.y = element_blank(), axis.ticks.y = element_blank())   
```
...
.pull-right[
* Die Auftretenswahrscheinlichkeit von $z = -1.77$ liegt im Verwerfungsbereich

* Zum Vergleich: kritischer Wert für $p=.05$ unter der Standardnormalverteilung $z_{krit}=-1.96$

Interpretation:
* 87 ist aureichend weit von $np=100$ weg
* Die $H_{0}$ wird verworfen
* Inhaltlich: Signifikant weniger als 50% der Frauen, die diäten, sind übergewichtig.
]
...
#### Beispiel 3: Binomialtest (zweiseitiges Testen) mit NV-Approximation

* Angenommen, die Psychologin hätte keine Vermutung in eine bestimmte Richtung gehabt und die $H_{1}$ ungerichtet formuliert:
...
* Kontrolle des $α$-Fehlers in beide Richtungen notwendig

$$P(Z \leq -1.77)+P(Z \geq 1.77)=0.0384+0.0384=0.0768> α = 0.05$$
* Ergebnis liegt noch im Annahmebereich der $H_{0}$
...
.pull-right[
* Bei beidseitigem Test auf $\alpha=.05$ liegen Auftretenswahrscheinlichkeiten von $z = -1.77$ und $z = 1.77$ nicht im Verwerfungsbereich

Interpretation:
* Die $H_{0}$ wird beibehalaten
* Inhaltlich: Signifikant weniger als 50% der Frauen, die diäten, sind übergewichtig.

* Es können bei denselben Daten unterschiedliche Entscheidungen resultieren. 
* Beim einseitigen Testen kommt man eher zu einer Verwerfung der Nullhypothese
]
...
* Um den kritischen Wert zu finden, musst zunächst das sogenannte Signifikanzniveau  $\alpha$ festgelegt werden
...
* Die Teststärke hängt von verschiedenen Faktoren ab:
  * Signifikanzniveau $α$ (je kleiner des weniger Power)
...
* Ergebnise eines Signifikanztests ist unmittelbar von der Stichprobengröße abhängig!

* Beispiel: Binomialtest mit NV-Approximation, Anteil der übergewichtigen Frauen

* Stichprobengröße $n = 2000$ Frauen (statt $n = 200$), gleicher Anteil wie vorher: 43.5% übergewichtig = 870.

* $μ=np=1000,k=870$

$$z=\frac{870.5-1000}{\sqrt{2000 \cdot0.50\cdot0.50}}=-5.79$$
$\rightarrow$ Vergleich zuvor $(n=200)$: $z=-1.77$ 
...
* Der Test wir somit strenger (Grenze für Signifikanz niedriger)
