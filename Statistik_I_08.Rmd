...
>Formel: $$F=\frac{\hat{\sigma}^2_1}{\hat{\sigma}^2_2}; df_1 = n_1 − 1, df_2 = n_2 − 1$$
...
ggplot(df, aes(x = PF, colour = Gruppe)) + 
 ...
 # scale_x_continuous(breaks = c(0,4.91,7.22, 10, 15)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mean(df$PF), sd = sd(df$PF)), colour = "#621C37") +
  stat_function(fun = dnorm, args = list(mean = mean(df2$PF), sd = sd(df2$PF)), colour = "#EE0071") +
...
ggplot(df, aes(x = PF, colour = Gruppe)) + 
 ...
 # scale_x_continuous(breaks = c(0,4.91,7.22, 10, 15)) +
  ...
  # geom_vline(xintercept = 4.91, linetype = "dashed", colour = "red") +
  stat_function(fun = dnorm, args = list(mean = mean(df$PF), sd = sd(df$PF)), colour = "#621C37") +
  stat_function(fun = dnorm, args = list(mean = mean(df2$PF), sd = sd(df2$PF)*1.5), colour = "#EE0071") +
  labs(x = "X", y = "relative Häufigkeit") +
   theme_classic() +
  theme(text = element_text(size = 25))
```
...
>Formel: $$t = \frac{\bar{x}_{1}- \bar{x}_{2}}{\sqrt{\frac{(n_{1} - 1) \cdot \hat{\sigma^2_{1}}+ (n_{2} - 1) \cdot \hat{\sigma^2_{2}}}{(n_{1} - 1) + (n_{2} - 1)} (\frac{1}{n_{1}}+\frac{1}{n_{2}})}}; df=n_{1}+n_{2} - 2$$
...
stat_summary(geom = "bar", colour = "#621C37") +
  geom_errorbar(aes(ymin = mean-se, ymax =mean+se), width = .2) +
  labs(y = "Verbesserung") +
  theme_classic() +
  scale_fill_manual(values = c("VG" = "#621C37",
                               "KG" = "#fdeef5")) +
...
#### Hedge’s g (oft auch einfach nur d geschrieben)
...
.pull-left[
>Formel: $$g=\frac{\bar{x}_1 - \bar{x}_2}{\sqrt{\frac{(n_1-1)  \cdot s^2_1 + (n_2-1)  \cdot s^2_2}{n_1 + n_2 - 2}}}$$
]
...
* Glass’s Δ wurde ursprünglich für experimentelle Studien entwickelt. Anstatt eine gepoolte oder Besselkorrigierte Standardabweichung zu verwenden, wird einfach die Standardabweichung der Kontrollgruppe verwendet. 
...
.pull-left[
>Formel: $$Δ=\frac{\bar{x}_1 - \bar{x}_2}{\sigma_{control}}$$
]
...
* Ergibt t-Test (Nullhypothesentest) eine geringe Wahrscheinlichkeit, ist Ablehnung der Nullhypothese möglich
...
$$t=\frac{\bar{x}_d}{\hat{\sigma}_{\bar{x}_d}}= \frac{\bar{x}_d}{\frac{\hat{\sigma}_d}{\sqrt{N}}}=\frac{0.722}{\frac{4.186}{\sqrt{36}}}=\frac{0.722}{0.698}=1.035$$
...
ggplot(df, aes(x = Gruppe, y = Reduktion)) +
   stat_summary(size = 1.5, geom = "bar", fill = "#fdeef5", color = "#621C37") +
  stat_summary(geom = "errorbar", fun.data = mean_se, width = .2, color = "#621C37") +
...
ggplot(df, aes(x = Messung, y = Reduktion)) +
   stat_summary(size = 1.5, colour = "#621C37") +
  stat_summary(geom = "line", group = 1, colour = "#621C37") +
...
.code60[
```{r}
t.test(df$Reduktion[df$Messung == "T1"], df$Reduktion[df$Messung == "T2"], paired = T)
```
]
]
