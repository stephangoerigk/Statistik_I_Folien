...
* Wir kennen unterschiedliche Variablenarten
  ...
  * manifest vs. latent
  * AV vs. UV
  * Drittvariablen: Stör- und Kontrollvariable, Moderator, Mediator
...
* Kombination **mehrerer** Variablen in einer **Datenmatrix**
...
* **Selten** in Sozialwissenschaften (Variablen wie Intelligenz, Neurotizismus... haben keine sinnvolle 0)
...
df = data.frame(Skalenniveau = c("Nominalskala", "Ordinalskala", "Intervallskala", "Verhältnisskala"),
                ...
                Transformationen = c("wenn \\(x_1\\) ungleich \\(x_2\\) dann \\(y_1\\) ungleich \\(y_2\\)", "wenn \\(x_1 > x_2 > x_3\\) dann \\(y_1 > y_2 > y_3\\)", "\\(y_i = a + b \\cdot x_i\\)", "\\(y_i = b \\cdot x_i\\)"))
...
* **Praxis:** es kommt vor, dass eingesetzte Messinstrumente Daten auf niedrigerem Skalenniveau erfassen, als theoretisch möglich (z.B. aus Sparsamkeit)
...
* **Einzelwert** des Items auf der Likertskala i.d.R. ordinalskaliert
...
.pull-left[
**Gängige Maße der zentralen Tendenz** *(Thema dieser Einheit)*:
...
.pull-right[
**Gängige Streuungsmaße** *(Vorausblick – Einheit 3)*:
...
**Aufgabe Mittelwert:** Berechnen Sie das Durchschnittsalter folgender Stichprobe (in Jahren) $(N=12)$
...
Lösungsweg: $$\bar{x}=\frac{1\cdot2+2\cdot1+3\cdot3+4\cdot7+5\cdot2+6\cdot5+7\cdot4+8\cdot1+9\cdot3+10\cdot2}{30}=\frac{`r  1*2+2*1+3*3+4*7+5*2+6*5+7*4+8*1+9*3+10*2`}{30} = `r round(mean(x), 2)`$$
...
> Formel: $$\bar{\bar{x}}=\frac{n_{1} \cdot \bar{x}_{1} + n_{2} \cdot \bar{x}_{2}}{n_{1} + n_{2}}$$
...
* Wert in der Mitte der Verteilung (halbiert die Verteilung)
...
.pull-right[
<svg width="100%" viewBox="0 0 380 190" style="display:block; margin-top:30px;">

  <!-- Median dashed line drawn first (behind boxes) -->
  <line x1="190" y1="46" x2="190" y2="108" stroke="#621C37" stroke-width="2" stroke-dasharray="4 3"/>

  <!-- Left half shaded (0–50): white base + pink tint = opaque -->
  <rect x="30" y="58" width="160" height="50" fill="white" rx="3"/>
  <rect x="30" y="58" width="160" height="50" fill="#EE0071" opacity="0.12" rx="3"/>
  <text x="110" y="78" text-anchor="middle" fill="#EE0071" font-size="11" font-weight="600" font-family="inherit">50% aller Werte</text>
  <text x="110" y="95" text-anchor="middle" fill="#EE0071" font-size="10" font-family="inherit">(untere Hälfte)</text>

  <!-- Right half shaded (50–100) -->
  <rect x="190" y="58" width="160" height="50" fill="white" rx="3"/>
  <rect x="190" y="58" width="160" height="50" fill="#EE0071" opacity="0.12" rx="3"/>
  <text x="270" y="78" text-anchor="middle" fill="#EE0071" font-size="11" font-weight="600" font-family="inherit">50% aller Werte</text>
  <text x="270" y="95" text-anchor="middle" fill="#EE0071" font-size="10" font-family="inherit">(obere Hälfte)</text>
...
* Bestimmung bei **gerader** Anzahl von Werten:
  ...
  * ordnen der Werte der Größe nach $\rightarrow$ numerische Mitte aus beiden mittleren Werten = Median
...
htmltools::tags$iframe(
  src = "outlier_mean_vs_median_v2.html",
  width = "80%",
  height = "2000px",
  frameborder = "0",
  scrolling = "no"
...
Beispiel für Robustheit gegenüber Extremwerten: $X_{10}$  ist ein Ausreißer
...
* **Histogramm** und **Balkendiagramm** eignen sich zur Darstellung der Verteilung einer Variable.

* Wir unterscheiden 4 Arten von **Skalenniveaus**: Nominal-, Ordinal-, Intervall- und Verhältnisskalenniveau.
