...
Somit können wir auf einfache Art und Weise Cohen’s d aus dem t-Wert berechnen
...
```{r echo=F}
df <- data.frame(
  Effektstärke = c("Kleiner Effekt", "Mittlerer Effekt", "Großer Effekt"),
  d = c("|0.2|", "|0.5|", "|0.8|")
)
...
kable(d, format = "html", escape = FALSE) %>%
  kable_styling(font_size = 18)
```
]
</small>
...
* Es bestimmt, wie groß die Stichprobe sein muss, um bei angenommener Effektstärke signifikante Ergebnisse zu erzielen und das Risiko eines Type-II-Fehlers zu minimieren.
...
* Eine Poweranalyse wird in der Regel computerisiert berechnet

* Für gängige statistische Tests wird oft folgende Software eingesetzt
